...
async def test_run_task_returns_expected_keys():
    ...
    with patch("kubedevaiops.agent.supervisor.get_supervisor", return_value=mock_agent):
...
async def test_reflect_returns_satisfactory():
    ...
    with patch("kubedevaiops.agent.supervisor.get_chat_model", return_value=mock_llm):
